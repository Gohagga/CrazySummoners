...
type SummonMeleeUnitData = {
    level: Record<number, {
        summonedUnitTypeId: number,
    }>;
}

export interface SummonMeleeAbilityData extends OrbAbilityData {
    levelSummonedUnitTypeId: Record<number, string>
}

export class SummonMelee extends OrbAbility {

    private unitConfigurable: UnitConfigurable<SummonMeleeUnitData>;
    private defaultAbilityData: SummonMeleeUnitData = {
        level: {
            1: {
    ...
    constructor(
        ...
        this.unitConfigurable = new UnitConfigurable<SummonMeleeUnitData>(() => this.defaultAbilityData);
    ...
    public GetUnitConfig = (unit: Unit, level?: number) => this.unitConfigurable.GetUnitConfig(unit).level[level || unit.getAbilityLevel(this.id)];
